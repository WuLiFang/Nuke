...
def fix_read():
    ...
    result = [
        _fix_one(n, filename_dict=_filename_dict()) for n in _nodes_that_has_error()
    ]
    result = {"success": result.count(True), "fail": result.count(False)}
    nuke.message("成功修复 {success} 个节点, 失败 {fail} 个".format(**result).encode("utf-8"))
    return result
...
def _fix_one(n, **context):
    ...
    context["node"] = n
    context["name"] = n.name()
    ret = _fix_thumbs_db(context) or _fix_missing_file(context) or _fix_dir(context)
...
def _filename_dict():
    filenames = set(_u_nuke_filename(n) for n in nuke.allNodes() if not n.hasError())
...
def _set_filename(n, value):
    try:
        k = (
            n["file"]
            if not n["proxy"].value() or nuke.value(b"root.proxy") == "false"
            else n["proxy"]
        )
    except NameError:
        k = n["file"]
...
def _fix_missing_file(context):
    ...
    if value:
        ...
        if not IS_TESTING and n.hasError():
            LOGGER.info(
                "修复读取: 使用新素材依旧出错, 撤销: " "{name}: {alter_filename}".format(**context)
            )
            _set_filename(n, filename)
            return False
        LOGGER.info(
            "修复读取: 使用同名文件: " "{name}: {filename} -> {alter_filename}".format(**context)
        )
        return True
    return False
