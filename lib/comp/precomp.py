...
class __PrecompSwitch(object):
    ...
    def get_which(cls, node):
        ...
        assert (
            isinstance(node, nuke.Node) and node.Class() == "Switch"
...
class Precomp(object):
    ...
    def __init__(self, nodes, renderer=None, async_=True):
        ...
        if len(self.source) == 1:
            n = sorted(
                nodes,
                key=lambda n: (len(nuke.layers(n)), len(_get_filename(n)) * -1),
                reverse=True,
            )[0]
            layers = nuke.layers(n)
            LOGGER.debug("Precomp single node that has this layers:\n%s", layers)
            self.source = {cast.text(layer): n for layer in layers}
            self.source["beauty"] = n
        LOGGER.debug("Source layers:\n%s", self.source.keys())

        # load config
        self._config = (
            detect_renderer(self.source.keys())
            if renderer is None
        ...
        remove_node = nuke.nodes.Remove(inputs=(dot_node,), channels="rgb")
        ...
        kwargs = {
            ...
            "onCreate": cast.binary(
                inspect.getsource(PrecompSwitch)
                + """
__PrecompSwitch.init(nuke.thisNode())"""
        ...
        _ = autoplace(self.last_node, recursive=True, undoable=False, async_=async_)
    ...
    def node(self, layer):
        ...
        if layer in self.layers():
            ...
            try:
                kwargs["postage_stamp"] = cast.not_none(self.last_node)[
                    b"postage_stamp"
                ].value()
            except NameError:
                pass
            ...
            if self.source.get(pair[0]) and self.source.get(pair[1]):
                LOGGER.debug("Combine for layer: %s, %s -> %s", pair[0], pair[1], layer)
                input0, input1 = self.node(pair[0]), self.node(pair[1])
                kwargs = {
                    "inputs": [input0, input1],
                    "operation": (self._config.combineMode.get(layer, "multiply")),
                    "output": "rgb",
