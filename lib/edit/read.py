...
def replace_sequence():
    ...
    _ = panel.addExpressionInput(label_first, (nuke.Root()[b"first_frame"].value()))
    _ = panel.addExpressionInput(label_last, (nuke.Root()[b"last_frame"].value()))

    confirm = panel.show()
    if confirm:
        render_path = os.path.normcase(cast.text(panel.value(label_path_prefix)))
...
def use_relative_path(nodes):
    ...
    for n in nodes:
        try:
            ...
            _ = n[b"file"].setValue(cast.binary(path.relative_to(proj_dir).as_posix()))
        except NameError:
            continue
...
def remove_duplicated_read(
    ...
    for n in nodes:
        ...
        if same_node:
            ...
            LOGGER.info(
                "用 {0} 代替 {1} , 删除 {1}。".format(cast.text(same_node.name()), n_name)
            )
            nuke.delete(n)
        else:
...
def _find_same(node, nodes):
    keys = ("file", "proxy", "first", "last", "frame_mode", "frame", "disable")
    try:
        return next(
            n for n in nodes if all(n[j].value() == node[j].value() for j in keys)
        )
