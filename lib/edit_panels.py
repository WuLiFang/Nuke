...
from edit import CurrentViewer, named_copy, replace_node, set_knobs, transfer_flags
...
class ChannelsRename(PythonPanel):
    ...
    def accept(self):
        ...
        n = named_copy(
            self._node, {channel: self[channel].value() for channel in self._channels}
        )
...
class MultiEdit(PythonPanel):
    ...
    def __init__(
        ...
        self.addKnob(
            nuke.Text_Knob(b"", cast.binary("以 {} 为模版".format(nodes[0].name())))
        )
        ...
        for k in knobs:
            ...
            if (
                isinstance(k, (nuke.Script_Knob, nuke.Obsolete_Knob))
        ...
        k.setFlag(nuke.STARTLINE)
        self.addKnob(k)
        self.addKnob(nuke.PyScript_Knob(b"cancel", b"Cancel", b"nuke.tabClose()"))
...
def same_class_filter(
    ...
    classes = list(
        set(
            [
                n.Class()
                for n in nodes
                if not node_class or n.Class() == cast.binary(node_class)
            ]
        )
    )
    classes.sort()
    if len(classes) > 1:
        choice = nuke.choice(
            cast.binary("选择节点分类"), cast.binary("节点分类"), classes, default=0
        )
        if choice is not None:
            nodes = [n for n in nodes if n.Class() == classes[choice]]
        else:
