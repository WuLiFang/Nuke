...
lib/site-packages/.make_sucess: .venv/.make_sucess requirements.txt
	...
	$(PYTHON27) -m pip install --target="lib/site-packages" -r "requirements.txt"  -c "python2-constraint.txt"
...
.venv/.make_sucess: .venv dev-requirements.txt docs/requirements.txt
	$(PYTHON27) -m pip install --target "$$(./scripts/get-venv-python-lib.sh)" -r dev-requirements.txt -r docs/requirements.txt -c python2-constraint.txt
